<template>
  <div class="hero">
    <div class="notification is-light is-danger" v-bind:class="resultAlertClass">
                    ...
                    <form @submit.prevent="searchMembers">
                        <div class="columns is-vcentered">
                            <div class="column is-one-quarter">
                                <label class="has-text-weight-semibold flex-center mr-4" for="payment-value">Data Inicial</label>
                                <Datepicker v-model="startDate" :enable-time-picker="false" :format="format" locale="pt" cancelText="Cancelar" selectText="Selecionar"/>
                            </div>
    
                            <div class="column is-one-quarter">
                                <label class="has-text-weight-semibold flex-center mr-4" for="payment-value">Data Final</label>
                                <Datepicker v-model="endDate" :enable-time-picker="false" :format="format" locale="pt" cancelText="Cancelar" selectText="Selecionar"/>
                            </div>

                            <div class="column is-one-quarter">
                                <label class="has-text-weight-semibold flex-center mr-4" for="payment-value">% de Ensaios</label>
                                <input type="number" v-model="percentageValue" class="input" placeholder="Digite apenas números" />
                            </div>
                            
                            <div class="column">
                                <button type="submit" class="button is-info is-fullwidth mt-5" v-bind:class="buttonLoading">Buscar</button>
                            </div>
                        </div>                        
                    </form>
            </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            filterBy(instrument) {
                return this.membersResult.filter( member => member.instrument == instrument)
            },
            filterAble(list) {
            ...
            setupCardTitles() {
                ...
                this.maestryCardTitle = 'Maestria ' + maestryAbleLength + ' / ' + this.maestryMembers.length + ' membros'
                this.cantoCardTitle = 'Canto ' + cantoAbleLength + ' / ' + this.cantoMembers.length + ' Membros'
                this.agbeCardTitle = 'Agbê ' + agbeAbleLength + ' / ' +this.agbeMembers.length + ' membros'
                this.agogoCardTitle = 'Agogô ' + agogoAbleLength + ' / ' +  this.agogoMembers.length + ' membros'
                this.gongueCardTitle = 'Gonguê ' + gongueAbleLength + ' / ' + this.gongueMembers.length + ' membros'
                this.caixaCardTitle = 'Caixa ' + caixaAbleLength + ' / ' + this.caixaMembers.length + ' membros'
                this.alfaiaCardTitle = 'Alfaia ' + alfaiaAbleLength + ' / ' + this.alfaiaMembers.length + ' membros'
            },
            searchMembers() {
                if (this.percentageValue == '' || this.startDate == null || this.endDate == null) {
